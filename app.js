...
function LoginScreen({ onLogin }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isSignUp) {
        const { data, error } = await SG_Auth.signUp(email, password);
        if (error) throw error;
        setSuccess('Account created! Please check your email for a confirmation link.');
      } else {
        const { data, error } = await SG_Auth.signIn(email, password);
        if (error) throw error;
  ...
  return React.createElement('div', { className: 'login-container' },
    React.createElement('div', { className: 'login-card' },
      React.createElement('h2', null, 'Glass Comparator Tool'),
      React.createElement('p', null, isSignUp ? 'Create your account' : 'Professional Comparison Engine'),
      React.createElement('form', { onSubmit: handleSubmit },
        React.createElement('div', { className: 'field-group' },
          React.createElement('label', { className: 'field-label' }, 'Email Address'),
          React.createElement('input', { type: 'email', className: 'field-select', value: email, onChange: e => setEmail(e.target.value), required: true })
        ),
        React.createElement('div', { className: 'field-group' },
          React.createElement('label', { className: 'field-label' }, 'Password'),
          React.createElement('input', { type: 'password', className: 'field-select', value: password, onChange: e => setPassword(e.target.value), required: true, minLength: 6 })
        ),
        error && React.createElement('div', { className: 'login-error' }, error),
        success && React.createElement('div', { className: 'login-success' }, success),
        React.createElement('button', { type: 'submit', className: 'btn-primary', style: { width: '100%', marginTop: '10px' }, disabled: loading },
          loading ? 'Processing...' : (isSignUp ? 'Create Account' : 'Sign In')
        )
      ),
      React.createElement('div', { className: 'login-switch' },
        React.createElement('span', null, isSignUp ? 'Already have an account? ' : 'First time here? '),
        React.createElement('button', { className: 'text-link', onClick: () => setIsSignUp(!isSignUp) },
          isSignUp ? 'Sign In instead' : 'Create an account'
        )
      ),
      React.createElement('div', { className: 'login-footer' }, 'Sales Team Portal')
    )
  );
}
...
function App() {
  ...
  useEffect(() => {
    SG_Auth.getSession().then(sess => {
      setSession(sess);
      if (sess) {
        GlassEngine.loadData().then(() => setDataLoaded(true));
      }
      setLoading(false);
    });

    const { data: { subscription } } = SG_Auth.onAuthStateChange((_event, sess) => {
      setSession(sess);
      if (sess && !dataLoaded) {
        GlassEngine.loadData().then(() => setDataLoaded(true));
      }
  ...
  if (loading) {
    return React.createElement('div', { className: 'app-container' },
      React.createElement('div', { className: 'loading-container' },
        ...
        React.createElement('div', { className: 'loading-text' }, 'Checking authentication...')
      )
    );
  }

  if (!session) {
    return React.createElement(LoginScreen, { onLogin: setSession });
  }
  ...
  return React.createElement('div', { className: 'app-container' },
    ...
    React.createElement('header', { className: 'header' },
      ...
      React.createElement('div', { className: 'header-actions' },
        searched && React.createElement('div', { className: 'export-section' },
          React.createElement('button', { className: 'export-btn', onClick: () => GlassEngine.exportPDF(results, target) }, '📄 PDF'),
          React.createElement('button', { className: 'export-btn', onClick: () => GlassEngine.exportExcel(results, target) }, '📊 Excel')
        ),
        React.createElement('button', { className: 'logout-btn', onClick: () => SG_Auth.signOut() }, 'Logout')
      )
    ),
    ...
    React.createElement('main', { className: 'main-content' },
      ...
      // ===== RESULTS =====
      searched && results.length > 0 && React.createElement('div', { style: { marginTop: 40 } },
