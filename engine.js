...
window.GlassEngine = (function() {
  ...
  function classify(product, target) {
    const lowerSHGC = (product.SHGC || 0) <= (target.SHGC || 0);
    const lowerU = (product.UValue || 0) <= (target.UValue || 0);
    const higherVLT = (product.VLT || 0) >= (target.VLT || 0);
    if (lowerSHGC && lowerU && higherVLT) return 'superior';
    const higherSHGC = (product.SHGC || 0) > (target.SHGC || 0);
    const higherU = (product.UValue || 0) > (target.UValue || 0);
    if (higherSHGC || higherU) return 'inferior';
    return 'closest';
  }
